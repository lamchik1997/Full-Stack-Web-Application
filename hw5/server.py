...
def build_OK_response(mineType, content, cookie=None):
    return buildResponseFromByte("200 OK", mineType, content, cookie)


# def build_set_cookie(cookie,visit):
#     response = "HTTP/1.1 200 OK\r\n"
#     response += "Content-Type: text/html" + mineType + "\r\n"
#     response += "Content-length: " + str(len(content)) + "\r\n"
#     response += "X-Content-Type-Options: nosniff" + "\r\n"
#     response += "\r\n"
#     return response, content

def build_404_response(mineType, content, cookie=None):
    return buildResponseFromByte("404 Not Found", mineType, content, cookie)
...
def buildResponseFromByte(response_code, mineType, content, cookie=None):
...
def decode_content_byte(path, cookie=None):
    ...
    file = open(path[1:], "rb")
    content = file.read()
    header, content = buildResponseFromByte('200 OK', content_type, content, cookie)
    file.close()
    return header, content


def decode_content(path, cookie=None):
    requestFile = path[1:]
    if path == '/comment' or path == '/image-upload':
        return build_301_response('index.html')
    if path == '/' or path == '/index.html' or path == '/login' or path == '/register':
        ...
        content_type = 'text/html'
        return build_OK_response(content_type, content, cookie)
        ...
        return build_404_response(content_type, "404 Page Not Found.\nThe path = " + requestFile, cookie)

    file = open(requestFile, "r")
    content = file.read()
    header, content = build_OK_response(content_type, content, cookie)
    file.close()
    return header, content
...
def htmlTemplate(sentence):
    ...
    <form action="/register" method="post" enctype="multipart/form-data">
                        <label for="text-form-username">username: </label>
                        <input id = "text-form-username" type="text" name="username" placeholder="Username" required><br/>
                        <label for="text-form-password">password: </label>
                        <input id="text-form-password"  type="text" name="password" placeholder="Enter Password" required><br/>
    ...
    <p>At least 3 or more unqiue character</p>
                        <label for="text-form-confirmpassword">confirm password: </label>
                        <input id="text-form-confirmpassword" type="text" name="confirmpassword" placeholder="Verify Password" required><br/>
                        <input type="submit" value="Submit">
    </form>
    <h2>Use the form below to login! </h2>
...
def check_uniqe(userpassword):
    uniqe = []
    for char in userpassword:
        if char not in uniqe and char.isalpha():
            uniqe.append(char)
    return len(uniqe)
...
def findCookie(header):
    print(header)
    if 'session' in header:
        header['session'] = "session=" + header['session']
    if 'Cookie' in header or 'id' in header:
        for x in userInfo.find():
            if 'Cookie' in x and 'session' in header and header['session'] == x['Cookie']:
                htmlTemplate("Welcome back, " + x['username'])
                return True
        for x in cookie_db.find():
            if 'Cookie' in x and header['Cookie'] == x['Cookie']:
                htmlTemplate("Welcome back new user")
                return True
    return False


def checkValidPassword(pw):
    regex = re.compile('[@_!#$%^&*()<>?/\|}{~:]')
    unique = check_uniqe(pw)
    if len(pw) < 8 or pw.islower() or pw.isupper() or pw.isalpha() or regex.search(
            pw) is None or int(unique) < 3:
        print("Password is not valid")
        return False
    return True


def storeToDB(finalDic):
    salt = bcrypt.gensalt()
    hashed = bcrypt.hashpw(finalDic['password'], salt)  # hash the password.
    # store the username and password in db.
    data = {"username": finalDic['username'], "password": hashed, "salt": salt}
    userInfo.insert(data)


def generateNewCookie():
    # generate random string for the cookie
    new_cookie = ''.join(random.choices(string.ascii_uppercase + string.digits, k=30))
    new_cookie = "id=" + new_cookie  # set key =ID
    data = {"Cookie": new_cookie}  # create dict for insertion to DB
    cookie_db.insert_one(data)  # insert cookie to cookie_db
    return new_cookie


def parseMultMedia(received_data):
    rn = "\r\n\r\n".encode()
    ...
    def handle(self):
        ...
        if request_type == 'GET':
            if request_path.endswith('.jpg') or request_path.endswith('.txt') \
                ...
                request_header = parse_header(received_data)  # get the GET request header.
                found = findCookie(request_header)  # find if the cookie is in db or not.
                if 'Cookie' not in request_header or not found:
                    new_cookie = generateNewCookie()
                    htmlTemplate("Welcome new user")
                    header, content = decode_content(request_path, new_cookie)
            ...
            if request_path == '/register':
                print("----------i'm at register section-----------")
                finalDic, status = parseMultMedia(received_data)

                if 'username' not in finalDic or 'password' not in finalDic or 'confirmpassword' not in finalDic:
                    message = "Missing username or password"
                    header, content = build_404_response('text/plain', message)
                    self.request.sendall(header.encode() + content.encode())
                else:
                    finalDic['username'] = finalDic['username'].decode()
                    password = finalDic['password'].decode()
                    for x in userInfo.find():
                        if 'username' in x and x['username'] == finalDic['username']:
                            header, content = build_404_response('text/plain', "Username already exited.")
                            self.request.sendall(header.encode() + content.encode())
                    if finalDic['password'] != finalDic['confirmpassword']:
                        header, content = build_404_response('text/plain', "Password doesn't match")
                        self.request.sendall(header.encode() + content.encode())
                    validated = checkValidPassword(password)  # This function will validate the password to see
                    # if it meet the requirement.
                    if not validated:
                        # edit the home page with the message inserted.
                        htmlTemplate("The password was not acceptable.Please try again")
                        header, content = decode_content('/index.html')
                        self.request.sendall(header.encode() + content.encode())
                    else:
                        storeToDB(finalDic)
                        htmlTemplate("You're sucessfully register.")
                        header,content = decode_content('/index.html')
                        self.request.sendall(header.encode() + content.encode())
            if request_path == '/login':
                print("------------I'm at login section--------------")
                finalDic, status = parseMultMedia(received_data)
                if 'username' not in finalDic or 'password' not in finalDic:
                    message = "Missing username or password"
                    header, content = build_404_response('text/plain', message)
                    self.request.sendall(header.encode() + content.encode())
                else:
                    finalDic['username'] = finalDic['username'].decode()  # Find the username in the db.
                    found = False
                    setCookie = True
                    for x in userInfo.find():
                        if 'username' in x and x['username'] == finalDic['username']:
                            ...
                            if 'Cookie' not in x:
                                ...
                                # cookie already set in DB. check if the header have the same cookie.
                                x_cookie = x['Cookie']
                                if 'session' in finalDic and x_cookie == finalDic['session']:
                                    print(x_cookie)
                                    setCookie = True
                                else:
                                    setCookie = False
                    if found:
                        # Compute the current password hash using the salt.
                        curHash = bcrypt.hashpw(finalDic['password'], salt)
                        if curHash == password:  # Compare if the hashed password in the db match with hashed password.
                            if not setCookie:  # set the session cookie.
                                new_cookie = ''.join(random.choices(string.ascii_uppercase + string.digits, k=30))
                                new_cookie = 'session=' + new_cookie
                                userInfo.update_many({'username': username}, {'$set': {'Cookie': new_cookie}})
                                message = "You're logged in! " + username
                                htmlTemplate(message)
                                header, content = decode_content("/index.html", new_cookie)
                                print("New login user ,set the cookie for this user.")
                                setCookie = True
                                self.request.sendall(header.encode() + content.encode())
                            else:
                                message = "You're logged back in! " + username
                                htmlTemplate(message)
